# New Job Template

## Offer

**Company:**

**Role:**

**Area:**

**Level:**

**Location / Remote:**

**Status:**

- Sourcing

**Priority:**

**Deadline:**

## Goal

Decide whether this is a target role, tailor the resume if needed, and prepare a strong application path.

## Fit Check

- Strong matches:
- Main gaps:
- Score:
- Decision:

## Matrix

| Requirement | Evidence | Gap | Action |
| --- | --- | --- | --- |
|  |  |  |  |

## Narrative

- Current experience thread:
- Why this role fits the thread:
- Best proof points:

## Action Plan

1. 
2. 
3. 
4. 

## Priority Analysis

- What matters most for this level:
- Core signals:
- Nice-to-have signals:
- What to ignore:

## Tailoring

- [ ] resume tailored
- [ ] LinkedIn updated if needed
- [ ] portfolio evidence selected
- [ ] cover note or message drafted
- [ ] referral or contact plan checked

## Submission

**Applied on:** Not yet

**Submitted via:** Not yet

**Materials used:** Master resume and target-specific proof set

## Interview Plan

- [ ] company research
- [ ] role research
- [ ] technical prep
- [ ] behavioral stories
- [ ] questions to ask

## Follow-Up

- [ ] follow-up date
- [ ] recruiter contact
- [ ] notes
- [ ] next action

## Results

**Outcome:** Not yet applied

**What worked:**

**What failed:**

**What to reuse next time:**

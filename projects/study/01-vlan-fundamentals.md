...
## 11. Interactive Review Log

### Core Definitions

- **VLAN vs subnet:** a VLAN is a Layer-2 broadcast domain; a subnet is a Layer-3 addressing boundary.
- **Access port:** a switch port assigned to one VLAN for a single endpoint.
- **Trunk port:** a switch port carrying multiple VLANs over one link.
- **ARP:** the protocol used to find the destination MAC address for a local IP address.
- **SCADA vs HMI:** SCADA is the supervisory layer; HMI is the operator screen or interface.
- **Firewall:** enforces security policy between zones by allowing, denying, and logging traffic.
- **Communication matrix:** a list of allowed device-to-device flows used to build firewall policy.

### OT Design Questions

- **Why do different VLANs need routing?** VLANs only segment Layer-2 traffic; different VLANs need a router or firewall to communicate.
- **Why is the firewall the default gateway in OT?** It forces inter-zone traffic through a controlled policy point.
- **Why is a flat network dangerous?** Traffic can bypass the firewall entirely, so security policy is not enforced.
- **Why is the Industrial DMZ important?** It buffers business IT and outside access from OT so OT is not directly exposed.
- **Does OT have general Internet access?** Usually no; OT should only reach approved internal services through controlled paths.
- **What is a jump host?** A hardened middle machine used as the first access point before reaching OT systems.
- **Why is a wrong subnet mask a problem?** The host misjudges what is local versus remote and may ARP or route incorrectly.
- **What is the risk of placing an HMI in the wrong VLAN?** It can expose the HMI to the wrong security zone and bypass intended controls.
- **What is the role of a switch port plan?** It maps each device to the correct switch port, VLAN, and port mode.
- **What is the role of a firewall communication matrix?** It defines and validates exactly which device-to-device flows are allowed.

### Troubleshooting Order

- check Layer 1 first
- verify gateway reachability
- confirm the correct VLAN on the switch
- confirm firewall policy
- verify subnet mask and addressing if routing behavior looks wrong

### Tight Interview Lines

- `VLANs separate traffic; routing enables communication between VLANs.`
- `The firewall applies policy to traffic between zones.`
- `The DMZ is a buffer zone between IT and OT.`
- `OT should not have general Internet access.`
- `A jump host is the controlled middle machine you use first.`
- `A communication matrix lists the allowed device-to-device flows.`
- `A flat network is dangerous because traffic can bypass the firewall.`
